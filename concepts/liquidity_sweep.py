...
def required_lookback_seconds(config, candle_seconds):
    ...
    guessing an interval, since collection granularity isn't fixed. Not
    the same as this module's own _detect_candle_seconds below, which
    runs inside compute() against already-normalized candles -- this one
    runs before compute() even starts, against raw records, to decide how
    much of the history compute() gets handed in the first place."""
